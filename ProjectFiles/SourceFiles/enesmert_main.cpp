...
int main() {
    ...
    // KidSpeaker ve Child cihazlarýný test et
    ...
    child1->cry(); // Çocuk aðlamaya baþlýyor
    kidSpeaker1->monitorChild(); // Çocuðu izlemeye baþla

    cout << "John is being soothed..." << endl;
    child1->sootheChild(); // Çocuk sakinleþiyor

    if (!child1->getChildState()) {
        cout << "John is calm, stopping KidSpeaker." << endl;
        kidSpeaker1->stopRinging(); // Çocuk sakinleþti, alarm durduruluyor
    }

    // TV, Thermostat, Fridge, Curtain cihazlarýný test et
    ...
    // Oven cihazýný test et
    int dummyAttr = 0; // Placeholder attribute, uygun þekilde deðiþtirilebilir
    ...
    kitchenOven.display(); // Fýrýný ekrana yazdýrma
